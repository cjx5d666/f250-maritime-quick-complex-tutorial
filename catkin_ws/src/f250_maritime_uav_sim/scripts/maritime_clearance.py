...
from maritime_scene_utils import dynamic_mode_enabled, dynamic_obstacle_center
...
def dynamic_planner_obstacles_at(scene, time_sec=0.0, dynamic_mode="auto"):
    ...
    for index, item in enumerate(scene_dynamic_obstacles(scene)):
        ...
        if shape == "box":
            obstacles.append(make_box_obstacle(
                name, center, item.get("size"), "dynamic", "dynamic_obstacles",
                yaw=float(item.get("yaw", 0.0))))
        elif shape == "cylinder":
            cylinder = dict(item)
            cylinder["center"] = center
            obstacles.append(make_cylinder_obstacle(name, cylinder, "dynamic", "dynamic_obstacles"))
        else:

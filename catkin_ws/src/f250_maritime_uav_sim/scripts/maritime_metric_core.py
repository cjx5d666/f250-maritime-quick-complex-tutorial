...
class MetricAccumulator:
    ...
    def _set_active_goal(self, index, time_sec):
        ...
        if previous == index:
            ...
            return
        if previous is not None and previous != index and 0 <= previous < len(self.stats):
            self.finalize_waypoint(previous, time_sec, "active_goal_switch")
    ...
    def summary(self):
        ...
        geometry_entries = static_geometry_entries + dynamic_geometry_entries
        cloud_entries = static_cloud_entries + dynamic_cloud_entries
        ...
        metric37_pass = static_safe and self.p8_completed
        metric38_pass = bool(
            self.max_active_goal_index >= len(self.stats) - 1 and self.p8_completed
        )
        metric39_pass = bool(self.p8_completed and self.waypoint_passed(len(self.stats) - 1))
        route_ok = bool(
            static_safe and self.p8_completed and metric36_pass and metric37_pass and
            metric38_pass and metric39_pass
        )
        return {
            "ok": route_ok,
            ...
            "metric_3_7": {
                "description": "No static planner-visible entry and P8 final hold completed; dynamic clearance is telemetry",
            ...
            "metric_3_8": {
                "description": "Planner advanced through the P0-P8 route and completed P8 final hold",
                "passed": metric38_pass,
                "max_active_goal_index": self.max_active_goal_index,
                "required_final_goal_index": len(self.stats) - 1,
            },
            "metric_3_9": {
                "description": "P8 final-hold nearest endpoint error divided by P0-P8 route length",
                "final_error_m": p8["nearest_distance_m"],
                "final_error_ratio": p8["metric_3_9_final_error_ratio"],
                "passed": metric39_pass,
            },
            "policy_note": (
                "P0-P8 route/planner evaluation excludes Metric 3.10 and yaw pass/fail. "

...
def metric37_min_clearance(metric37):
    clearance = metric37.get("clearance") or {}
    values = []
    for key in (
            "static_min_clearance_m",
            "static_cloud_min_distance_m",
            "dynamic_min_clearance_m",
            "dynamic_cloud_min_distance_m"):
...
def waypoint_terminal_block(stat, summary, status=None, reason=None):
    ...
    geometry_entries = int(m37["geometry_entry_count"])
    cloud_entries = int(m37["cloud_entry_count"])
...
def final_summary_terminal_block(summary):
    ...
    m37 = summary["metric_3_7"]
    m38 = summary["metric_3_8"]
    m39 = summary["metric_3_9"]
    lines = [
        "================ FINAL SUMMARY ================",
        "%-10s %s / hold %.3f sec" % (
            "P8",
            "completed" if summary["p8_completed"] else "not complete",
        ...
        "%-10s %s" % (
            "3.7 safe",
            pass_text(m37["passed"]),
        ),
        "%-10s %s" % (
            "3.8 plan",
            pass_text(m38["passed"]),
        ),
...
class MetricMonitorNode:
    ...
    def odom_callback(self, msg):
        ...
        if self.accumulator.p8_completed and not self.wrote_outputs:
    ...
    def global_label(self, marker_id):
        ...
        text = "3.6 mean %s | 3.7 %s | 3.8 %s | 3.9 %s | 3.10 FC-only" % (
            "--" if m36 is None else "%.5f" % m36,
            "SAFE" if summary["metric_3_7"]["safe_so_far"] else "UNSAFE",
            "PASS" if summary["metric_3_8"]["passed"] else "FAIL",

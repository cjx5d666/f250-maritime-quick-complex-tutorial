...
from maritime_clearance import dynamic_planner_obstacles_at, static_planner_obstacles
...
def lidar_understanding_markers(frame_id, start_id, odom, radius_m, local_range_xy, local_range_z, stamp=None):
    if odom is None:
        return []
    pose = odom.pose.pose
    ...
    return [
        make_line_marker(frame_id, start_id, "lidar_horizontal_scan_plane", points, color(0.1, 0.95, 1.0, 0.60), width=0.10, stamp=stamp),
        ...
        make_text_marker(frame_id, start_id + 3, "perception_note", "horizontal LiDAR returns feed EGO inflated map", (ox, oy, z + 2.0), color(0.75, 1.0, 1.0, 0.9), size=1.0, stamp=stamp),
    ]
...
def make_marker(frame_id, marker_id, obstacle, inflation_m, dynamic=False, stamp=None):
    marker = Marker()
    marker.header.frame_id = frame_id
    marker.header.stamp = stamp if stamp is not None else marker_stamp()
    marker.ns = "reference_dynamic_safety_envelope" if dynamic else "reference_static_safety_envelope"
    marker.id = marker_id
    marker.action = Marker.ADD
    marker.lifetime = rospy.Duration(0.0)
    ...
    marker.color = color(1.0, 0.22, 0.04, 0.18) if dynamic else color(1.0, 0.58, 0.05, 0.22)
    return marker


def build_markers(scene, frame_id, inflation_m, dynamic_mode, time_sec, stamp=None, odom=None, lidar_range_m=18.0, local_range_xy=18.0, local_range_z=9.0):
    ...
    for obstacle in static_planner_obstacles(scene):
        marker = make_marker(frame_id, marker_id, obstacle, inflation_m, dynamic=False, stamp=stamp)
        if marker is not None:
            markers.append(marker)
            marker_id += 1

    for obstacle in dynamic_planner_obstacles_at(scene, time_sec=time_sec, dynamic_mode=dynamic_mode):
        marker = make_marker(frame_id, marker_id, obstacle, inflation_m, dynamic=True, stamp=stamp)
        if marker is not None:
            markers.append(marker)
            marker_id += 1

    markers.extend(lidar_understanding_markers(frame_id, marker_id, odom, lidar_range_m, local_range_xy, local_range_z, stamp=stamp))
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        publisher.publish(build_markers(scene, frame_id, inflation_m, dynamic_mode, now - start_wall, odom=latest_odom["msg"], lidar_range_m=lidar_range_m, local_range_xy=local_range_xy, local_range_z=local_range_z))
        rate.sleep()

...
class QuickComplexRecorder:
    ...
    def summary(self):
        max_altitude = max((row[4] for row in self.rows), default=None)
        armed_rows = [row for row in self.rows if row[31] == "true"]
        min_altitude_armed = min((row[4] for row in armed_rows), default=None)
        cross_tracks = [row[38] for row in self.rows if isinstance(row[38], float)]
        along_tracks = [row[39] for row in self.rows if isinstance(row[39], float)]
        p0 = self.waypoints[0] if self.waypoints else None
        p8 = self.waypoints[-1] if self.waypoints else None
        ...
        def waypoint_metrics(waypoint):
            ...
            for row in self.rows:
                ...
                if first_within_t is None and dist <= radius:
                    first_within_t = row[0]
            return nearest, nearest_t, first_within_t

        p0_near, p0_near_t, p0_first_within = waypoint_metrics(p0)
        p8_near, p8_near_t, p8_first_within = waypoint_metrics(p8)
        scored_duration = None
        if p0_first_within is not None and p8_first_within is not None:
            scored_duration = max(0.0, p8_first_within - p0_first_within)
        ...
        if self.rows:
            ...
            current_span = 0.0
            for row in self.rows:
                dist = row[37] if isinstance(row[37], float) else None
                if previous is not None and dist is not None and previous is not None:
                    if dist > 3.0:
                        current_span += row[0] - previous
        ...
        if self.latest_active_goal_index != len(self.waypoints) - 1:
            errors.append("active goal never reached P8")
        if self.final_hold_seen_sec < self.final_hold_required:
        ...
        return {
            ...
            "duration_observed_sec": self.rows[-1][0] if self.rows else 0.0,
            ...
            "task": {
                ...
                "p0_wall_sec": p0_first_within,
                "p0_nearest_distance_m": p0_near,
                "p0_nearest_wall_sec": p0_near_t,
                "p0_first_within_radius_m": float(p0.get("radius", 1.0)) if p0 else None,
                "p8_nearest_distance_m": p8_near,
                "p8_nearest_wall_sec": p8_near_t,
                "p8_first_within_wall_sec": p8_first_within,
                "p8_first_within_radius_m": float(p8.get("radius", 1.0)) if p8 else None,
            },
            "landing": {
                "required": False,

...
路线/规划验收只看 P0-P8 路线任务，当前终端正式显示 `3.6`、`3.8` 和 `3.9`；不把 Metric 3.10、yaw pass/fail 或 Dsafe/static safety 作为路线终端指标。Metric 3.10 是独立的 FC-only 稳态证据。动态船只 clearance 和静态 clearance 仍写入 JSON/日志作审计与调试，不作为当前人机终端噪声输出。

2026-06-09 VM 实机重跑通过值：
...
路线终端会先逐点显示 `reached n/8` 和成功率；结束后再从离线 `metric_summary.json` 打印 3.6 / 3.9 实测误差，避免把途中 active-nearest 临时值当成最终指标。

2026-06-09 VM 实机重跑 FC Metric 3.10 通过值：

```text
3.10 e_pos mean = 0.371%
3.10 e_vel mean = 2.099%
3.10 e_att mean = 0.122%
3.10 control stability error = max(mean e_pos, mean e_vel, mean e_att) = 2.099%
result PASS
```

3.10 口径为 10 个 velocity 窗口、10 个 position 窗口和 10 个 yaw/e_att 窗口分别取均值，再取 `max(mean e_pos, mean e_vel, mean e_att)`。同日重复 FC 运行均 settled，但数值会随实时调度、图形负载和 PX4/Gazebo 状态小幅波动。
